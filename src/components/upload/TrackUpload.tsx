...
export function TrackUpload() {
    ...
    const fetchAlbums = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/artists/${user.uuid}/albums`,
          {
            credentials: 'include',
          }
        );
  ...
  const handleAudioUpload = async (file: File) => {
    try {
      ...
      const trackUrl = await response.json();
      ...
      toast({
        variant: "destructive",
        description: error instanceof Error ? error.message : "트랙 파일 업로드에 실패했습니다.",
      });
      setAudioFile(null);
    } finally {
      setIsLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        description: error instanceof Error ? error.message : "트랙 생성에 실패했습니다.",
      });
    } finally {
      setIsLoading(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-medium">앨범 선택</h2>
        ...
        <div className="space-y-6">
          <h2 className="text-lg font-medium">트랙 정보</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              ...
              <div>
                ...
                <Textarea
                  value={form.lyric}
                  onChange={(e) => setForm(prev => ({ ...prev, lyric: e.target.value }))}
                ...
                {audioFile ? (
                  <div className="absolute inset-0 flex items-center justify-center p-8">
                    <div className="text-center">
                      <p className="font-medium mb-2 break-all">{audioFile.name}</p>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                    <div className="p-4 rounded-full bg-white/5 group-hover:bg-white/10 transition-colors">
                      <Upload className="w-8 h-8 text-muted-foreground group-hover:text-white/60 transition-colors" />
                    </div>
                    <div className="text-center">
                      <p className="font-medium mb-1">트랙 파일을 업로드하세요</p>
